...
. eu_activate `eu_active_project`
. eu_gricad_run_experiment -n $NUM_PROCESSES $ARGUMENTS" > oar_job_run_experiment.sh
  chmod +x oar_job_run_experiment.sh

  echo "Register current experiment as a job ..."
  oarsub -l /nodes=1/gpu=$NUM_PROCESSES,walltime=$WALLTIME --project $PROJECT --stdout gricad_run_experiment.out.log --stderr gricad_run_experiment.err.log ./oar_job_run_experiment.sh
  echo "Finished."
fi
